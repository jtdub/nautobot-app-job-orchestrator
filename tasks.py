...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_JOB_ORCHESTRATOR_xxx
namespace = Collection("job_orchestrator")
namespace.configure(
    {
        "job_orchestrator": {
            "nautobot_ver": "3.0.0",
            "project_name": "job-orchestrator",
            "python_ver": "3.12",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.job_orchestrator.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.job_orchestrator.compose_http_timeout,
        "NAUTOBOT_VER": context.job_orchestrator.nautobot_ver,
        "PYTHON_VER": context.job_orchestrator.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.job_orchestrator.project_name}",
        f'--project-directory "{context.job_orchestrator.compose_dir}"',
    ]

    for compose_file in context.job_orchestrator.compose_files:
        compose_file_path = os.path.join(context.job_orchestrator.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.job_orchestrator.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.job_orchestrator.python_ver}...")
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.job_orchestrator.python_ver
    dockerfile_path = os.path.join(context.job_orchestrator.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=""):
    ...
    if constrain_nautobot_ver:
        ...
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.job_orchestrator.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {constrain_python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations job_orchestrator"
...
@task(
    help={
        "version": "Version of Job Orchestrator to generate the release notes for.",
...
def pylint(context):
    ...
    command = f"{base_pylint_command} job_orchestrator"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("job_orchestrator") / Path("migrations")
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 job_orchestrator/templates/"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.job_orchestrator.local):
